...
object ListCommand: CliktCommand(name = "list") {

    init {
        Commands.ALIASES["ls"] = listOf("list")
    }

    var DATE_FORMAT = SimpleDateFormat("yyyy-MM-dd HH:mm:ss")

    val directory by argument("directory")
        .file(canBeFile = false, canBeDir = true, mustExist = true, mustBeReadable = true)
        .defaultLazy { File(System.getProperty("user.dir")) }
    val showDate by option("--date", "-D", help = "Display the last modification date of the file").boolean().default(true)
    val showSize by option("--size", "-S", help = "Display the size of the file").boolean().default(true)

    fun processStyle(file: File, isSymbolicLink: Boolean, ansi: Ansi = ansi()) = ansi.also {
        if (file.isDirectory) it.fgBlue().bold()
        if (file.isHidden) it.a(Ansi.Attribute.ITALIC)
        if (isSymbolicLink) it.fgBrightCyan().boldOff()
    }
    ...
    override fun run() {
        ...
        val lastModifiedPlaceholder = if (showDate) files.maxOf { DATE_FORMAT.format(Date(it.lastModified())).length } else 0
        for (file in files) {
            val style = ansi()
            val prefix = buildString {
                if (showDate) append("${DATE_FORMAT.format(Date(file.lastModified()))}${" ".repeat(lastModifiedPlaceholder)}".subSequence(0..lastModifiedPlaceholder))
